...
<body>
	...
	<div class = "div-menu-formulario">
		<form action="unidad_controlador" methhod="get">
		 	<label>Unidades Académicas</label>
			<button style="height: 3vw;" class="button button1" type="submit" name="command" value="create">Unidad Académica</button>
 	...
 	<div class="div-tabla-2">	
		...
		<table border="1" class="table2">
		    ...
		    <c:forEach items="${unidades}" var="unidad" >
	            <tbody>
	            	<tr data-variable-id="${unidad.getIdUnidad()}">
	            		<td><c:out value="${unidad.getIdUnidad()}"/></td>
		                <td><c:out value="${unidad.getNombre()}"/></td>
		                <td><c:out value="${unidad.getTipo()}"/></td>
		                <td align="center">
	    ...
	    document.addEventListener("DOMContentLoaded", function () {
	        ...
	        editbuttons.forEach(function (edit_button) {
	            edit_button.addEventListener("click", function () {
	                ...
	                window.location.href = "unidad_controlador?command=edit&id=" + variableId;
	            });
	        });
	        ...
	        deletebuttons.forEach(function (delete_button) {
	            delete_button.addEventListener("click", function () {
	                ...
	                fetch("unidad_controlador?command=delete&id=" + variableId, {
	                    method: "DELETE",
	                    headers: {
